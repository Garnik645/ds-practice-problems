...
### Задание 4: Метод push

Реализуйте основную операцию вставки:

```cpp
void push(const T& value);    // Добавляет элемент в кучу
```
...
Реализуйте конструктор, принимающий вектор и преобразующий его в кучу:
...
std::vector<int> invalidHeap = {10, 30, 20, 50, 5};

bool result1 = MaxHeap<int>::isHeap(validHeap);    // true
bool result2 = MaxHeap<int>::isHeap(invalidHeap);  // false
```
